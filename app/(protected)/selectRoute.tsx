...
const selectRoute = (props: Props) => {
  ...
  const [routes, setRoutes] = useState<
    { id: number; route: { latitude: number; longitude: number }[] }[]
  >([]);
  ...
  const updateRoutes = async (orders: any, driverLocation: any) => {
    const res = await getRoute(orders, driverLocation);
    console.log("reposne is", res);
    if (!res) {
      setError("Failed fetch routes try again");
      return;
    }
    if (res.length > 0) {
      for (let i = 0; i < res.length; i++) {
        // Ensure routes exist
        const encodedPolyline = res[i].routes[0].overview_polyline.points; // Extract polyline
        ...
        setRoutes((prev) => [...prev, routeWithId]); // Store route in state
      }
    }
  };
  ...
  const onMarkerClick = (id: number) => {
    setSelectedRoute(id);
    // Additional actions like showing details of the clicked point or navigating to another screen
  };
  ...
  useEffect(() => {
    if (location && locations) {
      ...
      updateRoutes(blRoutes, location);
    }
  }, [location]);
  return (
    <>
          ...
          {routes.length > 1 &&
            routes.map((rt: any, i) => (
              <Polyline
                ...
                strokeWidth={5}
                strokeColor={selectedRoute == rt.id ? "#00f" : "#bebebe   "}
                onPress={() => onMarkerClick(rt.id)}
                tappable
              />
            ))}
        </MapView>
